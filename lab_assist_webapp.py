...
def run_fastapi(port=8013):
    uvicorn.run(app, host="0.0.0.0", port=port, log_level="info")

def get_args():
    parser = argparse.ArgumentParser(description="Start the QA service")
    parser.add_argument(
        "-p", "--port",
        type=int,
        default=8013,
        help="Port to run the server on (default: 8013)"
    )
...
if __name__ == "__main__":
    ...
    api_thread = threading.Thread(target=run_fastapi, args=(args.port,), daemon=True)
    api_thread.start()
    ...
    demo.launch(
        ...
        server_port=7860,
        share=True,
        root_path="/user/lead/proxy/7860"
    )